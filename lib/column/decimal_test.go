...
func TestDecimal_Write128_WithShopspringDecimal(t *testing.T) {
	...
	for _, scale := range scales {
		t.Run(fmt.Sprintf("scale:%d", scale), func(t *testing.T) {
			...
			data := []sdecimal.Decimal{
				sdecimal.Zero,
				sdecimal.NewFromInt32(-1),
				sdecimal.NewFromInt(9223372036854775807),
				sdecimal.NewFromInt(-9223372036854775808),
				sdecimal.NewFromBigInt(minD128, 0),
				sdecimal.NewFromBigInt(maxD128, 0),
			}
			...
			for _, attempt := range data {
				...
				if value := sdecimal.NewFromBigInt(bi, int32(-scale)); !attempt.Equal(value) {
